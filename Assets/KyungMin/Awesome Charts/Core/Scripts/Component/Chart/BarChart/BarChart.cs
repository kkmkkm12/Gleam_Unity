...
namespace AwesomeCharts {
    ...
    public class BarChart : AxisBaseChart<BarData> {
        ...
        private void UpdateBarChartPositioner () {
            ...
            if (!HVset)
                positioner.HVset = false;
            else
                positioner.HVset = true;
        ...
        private void UpdateVerticalAxisEntriesProvider () {
            ...
            if (!HVset)
            {
                ...
                verticalLabelsProvider.HVset = false;
                horizontalLabelsProvider.HVset = false;
                //positioner.HVset = false;
            }
            else
            {
                ...
                verticalLabelsProvider.labelsGravity = AxisConfig.VerticalAxisConfig.LabelsAlignment;
                verticalLabelsProvider.valueFormatter = GetCorrectVerticalAxisValueFormatter();
                verticalLabelsProvider.axisLength = verticalAxisLabelRenderer.GetComponent<RectTransform>().sizeDelta.x;
                verticalLabelsProvider.HVset = true;
                horizontalLabelsProvider.HVset = true;
                //positioner.HVset = true;
            }
        }
        ...
        private int UpdateBars (int dataSetIndex, int nextBarInstanceIndex) {
            ...
            for (int i = 0; i < barEntries.Count; i++) {
                UpdateBarWithEntry (barInstances[nextBarInstanceIndex],
                    ...
                    dataSetIndex);
                //Debug.Log("포지션 : " + barEntries[i].Position + ", 값 : " + barEntries[i].Value);
        ...
        void AddNewDataToChart()
        {
            ...
            barEntries.Add(new BarEntry(position: 3, value: 40));

            // Step 2: 새로운 BarDataSet 생성 (필요한 경우 제목과 색상을 설정)
            BarDataSet newDataSet = new BarDataSet("New DataSet", barEntries);

            // Step 3: 기존 BarData에 새로운 DataSet 추가
            data.DataSets.Add(newDataSet);
