...
namespace OSMClient
{
    ...
    public class SimpleGestures : MonoBehaviour
    {
        ...
        [SerializeField] RectTransform RawImage_Rect;
        ...
        private void MouseSimulation(List<Touch> touches)
        {
            ...
            bool onTheMap = RectTransformUtility.RectangleContainsScreenPoint(RawImage_Rect, mousePosition, SecondCamera);

            
            //곽경민 : 멀티 디스플레이로 빌드시 마우스 좌표 수정   (1번모니터 오른쪽에 SecondCanvas있을때 조건)
            if (Display.displays.Length > 1 && SecondCamera.targetDisplay == 1)
            {
                mousePosition.x -= Display.displays[0].systemWidth;
            }
            onTheMap = RectTransformUtility.RectangleContainsScreenPoint(RawImage_Rect, mousePosition, SecondCamera);

            //곽경민 : 지도 위에 마우스 있을 시 클릭하여 지도 내 위치 이동
            if (SimulateTouchWithMouse && (Input.GetMouseButton(0) || Input.GetMouseButton(1)) && onTheMap)
            {
                touches.Add(new Touch { position = mousePosition });
            }

            //곽경민 : 지도 위에 마우스 있을 시 휠 돌려 확대 축소
            if (SimulateScaleWithMouseWheel && Input.mouseScrollDelta.y != 0f && onTheMap)     //카메라 확대 부분 조건 추가
            {
